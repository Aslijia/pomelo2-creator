...
(function (Protobuf) {
    ...
    (function (MsgEncoder) {
        ...
        function checkMsg(msg, protos) {
            ...
            for (var name_1 in protos) {
                var proto = protos[name_1];
                //All required element must exist
                switch (proto.option) {
                    case 'required':
                        if (typeof (msg[name_1]) === 'undefined') {
                            console.warn('no property exist for required!', { name: name_1, proto: proto, msg: msg });
                            return false;
                        }
                    case 'optional':
                        if (typeof (msg[name_1]) !== 'undefined') {
                            var message_1 = protos.__messages[proto.type] || Protos['message ' + proto.type];
                            if (!!message_1 && !checkMsg(msg[name_1], message_1)) {
                                console.warn('inner proto error!', { name: name_1, proto: proto, msg: msg });
                                return false;
                            }
                        }
                        break;
                    case 'repeated':
                        ...
                        var message = protos.__messages[proto.type] || Protos['message ' + proto.type];
                        if (!!msg[name_1] && !!message) {
                            for (var i = 0; i < msg[name_1].length; i++) {
                                if (!checkMsg(msg[name_1][i], message)) {
                                    return false;
                                }
                            }
                        }
                ...
                if (!!protos[name_2]) {
                    ...
                    switch (proto.option) {
                        ...
                        case 'optional':
                            ...
                            offset = encodeProp(msg[name_2], proto.type, offset, buffer, protos);
                            break;
                        case 'repeated':
                            if (msg[name_2].length > 0) {
                                offset = encodeArray(msg[name_2], proto, offset, buffer, protos);
                            }
                            break;
                    }
                }
        ...
        function encodeProp(value, type, offset, buffer, protos) {
            switch (type) {
                ...
                case 'string':
                    var length_1 = Codec.byteLength(value);
                    //Encode length
                    offset = writeBytes(buffer, offset, Codec.encodeUInt32(length_1));
                    //write string
                    Codec.encodeStr(buffer, offset, value);
                    offset += length_1;
                    break;
                default:
                    var message = protos.__messages[type] || Protos['message ' + type];
                    if (!!message) {
                        //Use a tmp buffer to build an internal msg
                        var tmpBuffer = new Uint8Array(Codec.byteLength(JSON.stringify(value)) * 2);
                        var length_2 = 0;
                        length_2 = encodeMsg(tmpBuffer, length_2, message, value);
                        //Encode length
                        offset = writeBytes(buffer, offset, Codec.encodeUInt32(length_2));
                        //contact the object
    ...
    (function (MsgDecoder) {
        ...
        function decodeMsg(msg, protos, length) {
            while (offset < length) {
                ...
                switch (protos[name_3].option) {
                    case 'optional':
                    case 'required':
                        msg[name_3] = decodeProp(protos[name_3].type, protos);
                        break;
                    case 'repeated':
                        if (!msg[name_3]) {
                            msg[name_3] = [];
                        }
                        decodeArray(msg[name_3], protos[name_3].type, protos);
                        break;
                }
            }
            return msg;
        ...
        function decodeProp(type, protos) {
            switch (type) {
                case 'boolean':
                    var boolean = Codec.decodeStr(buffer, offset, 1);
                    offset++;
                ...
                case 'object':
                case 'string':
                    var length_3 = Codec.decodeUInt32(getBytes());
                    var str = Codec.decodeStr(buffer, offset, length_3);
                    offset += length_3;
                    if (type === 'object') {
                ...
                default:
                    ...
                    if (!!message) {
                        var length_4 = Codec.decodeUInt32(getBytes());
                        var msg = {};
                        decodeMsg(msg, message, offset + length_4);
                        return msg;
                    }
                    break;
            }
        ...
        function decodeArray(array, type, protos) {
            if (isSimpleType(type)) {
                var length_5 = Codec.decodeUInt32(getBytes());
