	...
	export namespace MsgEncoder {
			...
			for (let name in protos) {
				...
				switch (proto.option) {
					...
					case 'optional':
						if (typeof msg[name] !== 'undefined') {
							const message =
								protos.__messages[proto.type] ||
								Protos['message ' + proto.type]
		...
		function encodeProp(
			...
			switch (type) {
				...
				default:
					const message =
						protos.__messages[type] || Protos['message ' + type]
					if (!!message) {
						//Use a tmp buffer to build an internal msg
						const tmpBuffer = new Uint8Array(
							Codec.byteLength(JSON.stringify(value)) * 2
						)
						let length = 0
		...
		function encodeArray(
			...
			if (isSimpleType(proto.type)) {
				...
				offset = writeBytes(
					buffer,
					offset,
					Codec.encodeUInt32(array.length)
				)
