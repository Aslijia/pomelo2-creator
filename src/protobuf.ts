        ...
        function checkMsg(msg: any, protos: any) {
            ...
            for (let name in protos) {
                ...
                switch (proto.option) {
                    ...
                    case 'optional':
                        if (typeof (msg[name]) !== 'undefined') {
                            const message = protos.__messages[proto.type] || Protos['message ' + proto.type];
        ...
        function encodeProp(value: any, type: string, offset: number, buffer: Uint8Array, protos: any) {
            switch (type) {
                ...
                case 'string':
                    ...
                    break;
                default:
                    const message = protos.__messages[type] || Protos['message ' + type];
                    if (!!message) {
                        //Use a tmp buffer to build an internal msg
                        const tmpBuffer = new Uint8Array(Codec.byteLength(JSON.stringify(value)) * 2);
                        let length = 0;
        ...
        function decodeProp(type: string, protos: any) {
            switch (type) {
                ...
                case 'string':
                    ...
                    return str;
                default:
                    const message = protos && (protos.__messages[type] || Protos['message ' + type]);
                    if (!!message) {
                        const length = Codec.decodeUInt32(getBytes());
                        const msg = {};
