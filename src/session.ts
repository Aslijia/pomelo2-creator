...
export class Session extends EventEmitter {
	...
	private async connect() {
		...
		this.socket.on('connected', () => {
			...
			if (this.socket) {
				this.socket.send(
					Protocol.Package.encode(
						Protocol.PackageType.TYPE_HANDSHAKE,
	...
	async request(route: string, msg: object) {
		...
		if (body) {
			await this.socket.send(
				Protocol.Package.encode(Protocol.PackageType.TYPE_DATA, body)
			)
		}
	...
	private onHandshake(body?: Uint8Array | null) {
		...
		if (this.socket)
			this.socket.send(
				Protocol.Package.encode(Protocol.PackageType.TYPE_HANDSHAKE_ACK)
			)
		else
			this.logger.error(
				'TYPE_HANDSHAKE_ACK failed by socket is gone!',
				{}
			)
	}
	...
	private onHeartbeat() {
		...
		this.heartbeatId = setTimeout(() => {
			...
			if (this.socket) {
				this.socket.send(
					Protocol.Package.encode(Protocol.PackageType.TYPE_HEARTBEAT)
				)
			}
	...
	private _encode(reqId: number, route: string, msg: any) {
		...
		return Protocol.Message.encode(
			reqId,
			reqId
				? Protocol.MessageType.TYPE_REQUEST
				: Protocol.MessageType.TYPE_NOTIFY,
			!!this.dict[route],
